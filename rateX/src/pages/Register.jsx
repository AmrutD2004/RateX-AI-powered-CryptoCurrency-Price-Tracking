  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(data.message || "Something went erong")
      }
    } catch (error) {
      console.log(error)
      setLoading(false)
    ...
    <div className='min-h-screen flex items-center justify-center'>
      <div className='max-w-md bg-linear-to-tl from-[#FFEDD4] via-[#F7F3F1] to-[#FFEDD4] rounded-2xl shadow-xl p-8'>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col items-start'>
          ...
          <label className='text-neutral-600 text-lg mt-5 font-mono'>Username:</label>
          <input required onChange={handleChange} type="text" value={formData.userName} name='userName' className='px-4 py-2 rounded-lg bg-white w-full border border-neutral-300' placeholder='Enter username' />
          ...
          {loading ? (
            <button className='w-full mt-5 px-2 py-2 bg-[#3A2F3B] text-lg text-slate-300 rounded-lg border border-neutral-300 flex items-center justify-center gap-1'><Loader2 className='text-white animate-spin' />Register</button>
          ) : (
